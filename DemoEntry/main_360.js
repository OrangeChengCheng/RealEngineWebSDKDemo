...
function RealBIMLoadMainSce(e) {
  ...
  if (isSuccess) {
    ...
    //加载CAD全景
    // BlackHole3D.REloadCAD("https://yingshi-bim-demo-api.bosch-smartlife.com:8088/api/autoconvert/EngineRes/RequestEngineRes?dir=url_res02&path=3a078cdabb2e98a3b98e1960acfa15c6/1/638041974736297275.dwg", BlackHole3D.RE_CAD_UNIT.Millimeter, 1.0);
...
function MainSceDown(e) {
  ...
  if (e.detail.succeed) {
    ...
    // // 设置360相机与主相机是否同步
    // BlackHole3D.REsetViewSyn(1);
    // // 获取360相机与主相机是否同步
    // BlackHole3D.REgetViewSyn();
    // // 设置是否固定主相机
    // BlackHole3D.REisFixMainCam(0, [0, 0, 0], [0, 0, 0, 1]);
    // // 设置360相机朝向
    // BlackHole3D.RElocatePanCamToMainDir("left", 0);
    // BlackHole3D.REloadOverViewForCADByPath("http://realbim.bjblackhole.cn:8008/default.aspx?dir=url_res02&path=res_cad/103-Floor Plan - 三层建筑平面图.dwg","RE_CAD_UNIT.Millimeter",1.0);
    // BlackHole3D.REloadOverViewForCADByPath("https://yingshi-bim-demo-api.bosch-smartlife.com:8088/api/autoconvert/EngineRes/RequestEngineRes?dir=url_res02&path=3a078cdabb2e98a3b98e1960acfa15c6/1/638041974736297275.dwg", "RE_CAD_UNIT.Millimeter", 1.0);
...
function RealBIMLoadMinMapCAD(e) {
  if (e.detail.succeed == 1) {
    ...
    // BlackHole3D.REsetOverViewCamTransformInfoByPosMap(re_BIMPoints, re_CADPoints, re_CADUnit);//设置概略图相机变换数据 (通过顶点映射)
    var pandata = BlackHole3D.REgetPanSceElemInfos("pro360");
